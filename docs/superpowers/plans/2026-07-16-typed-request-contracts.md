...
> **STATUS: ✅ COMPLETE (2026-07-16).** All 5 tasks on `refactor/typed-request-contracts`. 13 request bodies (11 Map + createRoom entity + ScheduleRequest Map<Object>) → nested @Valid records across 6 controllers; 5 controllers migrated from raw X-User-Id to @AuthenticatedUser + @RequireAuth (requireRole role checks preserved — no privilege escalation). Whole-branch review verified all 13 records' JSON keys against the Flutter Dio calls (no drift; createRoom `isPrivate` kept via @JsonProperty), the createRoom mass-assignment/500 fix, validation-semantics parity, and auth role-check preservation. 602 chat-service tests green; frontend untouched. Deviations: Jackson coerces a numeric chatRoomId to String (ClassCastException path gone even more robustly than planned); MentionDigest/Scheduled X-Username null-guards were dead defensive code (removal benign).
