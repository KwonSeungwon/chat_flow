...
> **STATUS: PARTIAL (2026-07-05).** Branch `refactor/tier1-hotpath-perf`, subagent-driven (implementer + spec + code-quality review each) + whole-branch final review (READY TO MERGE). **Shipped:** #3 ReadReceipt SCAN→per-room hash (T1.1), #5 gateway JWT single-parse + dead `isValid` removal (T1.2), #6 frontend history-prepend scroll anchor (T1.4), #1 STOMP send-path duplicate `room_members` lookup collapsed (T1.5). Backend all-module suite green; frontend 232 tests + analyze clean. **Deferred to their own plans** (see below): #2 unread-count durable cursor (Group-B per-room cutoffs aren't batchable in JPQL and a native `unnest` query isn't testable on the H2 test profile — needs a durable DB cursor + Testcontainers-Postgres), #4 mention digest redesign (schema migration; also a correctness bug — `content LIKE '%@user%'` silently returns nothing under at-rest encryption). Review-surfaced follow-ups: gateway BCrypt off the Netty event loop; ReadReceipt write-path fault tolerance; `ChatService.processMessage(ChatMessage)` single-arg now has no production caller (harmless).
