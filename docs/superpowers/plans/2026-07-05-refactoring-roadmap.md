...
> **STATUS: ✅ COMPLETE (2026-07-13).** Initial pass 2026-07-05 (`refactor/tier1-hotpath-perf`): #3 ReadReceipt SCAN→per-room hash (T1.1), #5 gateway JWT single-parse + dead `isValid` removal (T1.2), #6 frontend history-prepend scroll anchor (T1.4), #1 STOMP send-path duplicate `room_members` lookup collapsed (T1.5). Follow-up passes 2026-07-13: gateway BCrypt off the Netty event loop + ReadReceipt write-path fault tolerance + V10 OWNER backfill (`refactor/review-followups`); **#4 mention digest redesign shipped** (`refactor/mention-redesign`, plan 2026-07-13-mention-digest-redesign.md — message_mentions table, encryption-safe, member-scoped, V11); **#2 unread durable cursor shipped** (`refactor/unread-durable-cursor`, plan 2026-07-13-unread-durable-cursor.md — room_members.last_read_at, V12, single grouped query; the original "not batchable in JPQL" blocker dissolved once the cursor moved into the table). Remaining minor: `ChatService.processMessage(ChatMessage)` single-arg has no production caller (harmless); mention re-extraction on message edit is an open design follow-up.
